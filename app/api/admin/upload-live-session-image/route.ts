/**
 * POST /api/admin/upload-live-session-image
 * Body: FormData con `file` (jpeg/png/webp ≤5MB) + `sessionId?`
 * Returns: { ok: true, url } — URL pública del cover subido.
 *
 * Reutiliza bucket `section-posters` con prefijo `live-sessions/…`
 * para no tener que crear un bucket nuevo. Path incluye timestamp
 * (o sessionId cuando estamos editando una existente).
 */
...
export async function POST(req: NextRequest) {
  ...
  const sessionIdRaw = fd.get("sessionId");
  const sessionId =
  ...
  const path = `live-sessions/${sessionId}/${Date.now()}.${ext}`;
